...
# draft data
lakeStations2020 <- read_csv('processedStationData/draft2020data/RegionalResultsLake_BRRO.csv') %>%
  mutate(ID305B = ID305B_1) # make joining column


# Old data, for double checking
...
# Some Lake names don't match between two dataframes so fix them manually as
#  well as the VAC/VAW changes

# Filter out problems for now
lakeStations_2 <- filter(lakeStations, !is.na(TPhosphorus_limit))


# find stations that are in the lacustrine zone based on last cycle's final lake station dataset
lakeStations_3 <- left_join(lakeStations_2, dplyr::select(lakeStationsFinal,FDT_STA_ID, Assess_TYPE), by='FDT_STA_ID')
