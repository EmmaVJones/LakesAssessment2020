...
# draft data
lakeStations2020 <- read_csv('processedStationData/final2020data/RegionalResultsLake_BRRO_EVJ.csv') %>%
...
lakeStations2020_3 <- filter(lakeStations2020_3, !is.na(SIGLAKENAME)) %>%
  rename('ID305B'='ID305B.x', 'REGION.y'='REGION',"WATER_NAME.y" ="WATER_NAME" ) %>% select(-c(ID305B_new, ID305B.y))  

# fix last problem sites, join by GNIS_Name
lakeStations2020_5 <- select(lakeStations2020_4,-c(ID305B_new:CYCLE)) %>%
  ...
  select(names(lakeStations2020_2))
...
#View(filter(lakeStations, is.na(TPhosphorus_limit)))
...
write.csv(lakeStations_3,'processedStationData/final2020data/lakeStations2020_BRRO.csv', row.names=F)
